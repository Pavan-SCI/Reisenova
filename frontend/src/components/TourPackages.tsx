    ...
    const ctx = gsap.context(() => {
      ...
      cards.forEach((card: any, i) => {
        ...
        gsap.fromTo(imgContainer,
          { clipPath: 'inset(100% 0% 0% 0% round 16px)', scale: 0.8, rotateX: 10, z: -200 },
          {
        ...
        // Image parallax removed for full view
        /* gsap.fromTo(img, 
          { yPercent: -8 },
          {
            yPercent: 8,
            ...
            >
              ...
              <div className="w-full lg:w-3/5 perspective-[1200px]">
                <div className="img-container w-full h-[500px] lg:h-[750px] overflow-hidden rounded-2xl relative shadow-[0_20px_50px_-10px_rgba(0,0,0,0.15)] transform-style-3d hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.25)] transition-shadow duration-700 group">
                  <img 
                    src={pkg.img} 
                    alt={pkg.title} 
                    className="w-full h-full inset-0 absolute object-cover will-change-transform group-hover:scale-105 transition-transform duration-1000 ease-out"
                  />
              ...
              <div className="w-full lg:w-2/5 flex flex-col justify-center perspective-[1200px] transform-style-3d" style={{ transform: 'translateZ(60px)' }}>
                ...
                <div className="overflow-hidden">
                  <Link to={`/packages/${pkg.id || pkg.title.toLowerCase().replace(/ & /g, '-and-').replace(/ /g, '-')}`} onClick={(e) => e.stopPropagation()} className="exp-text inline-flex items-center gap-4 group">
